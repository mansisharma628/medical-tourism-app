...
app.secret_key = os.environ.get('SECRET_KEY', 'medi_go_ease')

# Configure Resend and Admin Email
resend.api_key = os.environ.get("RESEND_API_KEY")
ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL", "medigoease@gmail.com")
...
def hospitals():
    conn = get_db_connection()
    hospitals_list = conn.execute('SELECT * FROM hospitals').fetchall()
    conn.close()
...
def contact():
    if request.method == 'POST':
        ...
        # 1. Save Inquiry to SQLite Database
        try:
            conn = get_db_connection()
            conn.execute('''
...
def admin_dashboard():
    conn = get_db_connection()
    leads = conn.execute('SELECT * FROM inquiries ORDER BY date DESC').fetchall()
...
def update_status(lead_id):
    ...
    conn = get_db_connection()
    conn.execute('UPDATE inquiries SET status = ? WHERE id = ?', (new_status, lead_id))
    conn.commit()
    conn.close()
...
def add_doctor():
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO doctors (name, specialty, experience, background) VALUES (?, ?, ?, ?)',
...
def add_hospital():
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO hospitals (name, location, description) VALUES (?, ?, ?)',
...
def add_testimonial():
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO testimonials (patient_name, country, review) VALUES (?, ?, ?)',
